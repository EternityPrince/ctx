...
func (s *shellSession) renderSymbolJourney(view storage.SymbolView) error {
	...
	if _, err := fmt.Fprintf(
		s.stdout,
		"%s\n  %s %s\n  %s %s\n  %s use %s to travel through symbols in the same file\n\n",
...
func (s *shellSession) renderFileJourney(filePath, focusSymbolKey string, symbols []storage.SymbolMatch) error {
	if _, err := fmt.Fprintf(
		s.stdout,
