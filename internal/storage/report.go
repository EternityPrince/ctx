...
func (s *Store) loadTopFunctions(snapshotID int64, limit int) ([]RankedSymbol, error) {
	...
	for rows.Next() {
		...
		item.Score = item.CallerCount*5 + item.ReferenceCount*2 + item.TestCount*3 + item.ReversePackageDeps*2
...
func (s *Store) loadTopTypes(snapshotID int64, limit int) ([]RankedSymbol, error) {
	rows, err := s.db.Query(`
		...
		FROM symbols s
		WHERE s.snapshot_id = ? AND s.kind IN ('struct', 'interface', 'type', 'alias')
