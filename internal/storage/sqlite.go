...
func (s *Store) CurrentFiles() (map[string]indexer.PreviousFile, error) {
	snapshot, ok, err := s.CurrentSnapshot()
	if err != nil || !ok {
		return map[string]indexer.PreviousFile{}, err
	}
	...
	files := make(map[string]indexer.PreviousFile)
	for rows.Next() {
		var record indexer.PreviousFile
		var isTest int
...
func (s *Store) SnapshotByID(id int64) (SnapshotInfo, error) {
	...
	var createdAt string
	err := s.db.QueryRow(`
		SELECT id, parent_id, kind, created_at, changed_files, changed_packages, changed_symbols,
...
func (s *Store) CommitSnapshot(kind, note string, scanned []indexer.ScanFile, result *indexer.Result, changes indexer.ChangeSet, full bool) (SnapshotInfo, error) {
	current, ok, err := s.CurrentSnapshot()
	if err != nil {
	...
	_, err = tx.Exec(`
		...
		WHERE id = ?
	`,
		len(changes.Added)+len(changes.Changed)+len(changes.Deleted),
...
func insertFiles(tx *sql.Tx, snapshotID int64, modulePath, root string, scanned []indexer.ScanFile) error {
	stmt, err := tx.Prepare(`
...
func insertPackages(tx *sql.Tx, snapshotID int64, packages []indexer.PackageFact) error {
	stmt, err := tx.Prepare(`
...
func insertSymbols(tx *sql.Tx, snapshotID int64, symbols []indexer.SymbolFact) error {
	stmt, err := tx.Prepare(`
		INSERT INTO symbols (
...
func insertDependencies(tx *sql.Tx, snapshotID int64, deps []indexer.DependencyFact) error {
	stmt, err := tx.Prepare(`
...
func insertReferences(tx *sql.Tx, snapshotID int64, refs []indexer.ReferenceFact) error {
	stmt, err := tx.Prepare(`
...
func insertCalls(tx *sql.Tx, snapshotID int64, calls []indexer.CallFact) error {
	stmt, err := tx.Prepare(`
...
func insertTests(tx *sql.Tx, snapshotID int64, tests []indexer.TestFact) error {
	stmt, err := tx.Prepare(`
...
func insertTestLinks(tx *sql.Tx, snapshotID int64, links []indexer.TestLinkFact) error {
	stmt, err := tx.Prepare(`
